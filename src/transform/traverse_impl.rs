...
impl<'a> Traverse<'a, ()> for DomExpressions<'a> {
    ...
    fn exit_program(&mut self, program: &mut Program<'a>, _ctx: &mut TraverseCtx<'a, ()>) {
        ...
        if !self.templates.is_empty() {
            ...
            for (html, var_name) in &self.template_map {
                let template_call = self.create_template_call(html);
                ...
                let declarator = VariableDeclarator {
                    ...
                    id: BindingPattern {
                        kind: BindingPatternKind::BindingIdentifier(Box::new_in(
                            BindingIdentifier {
                                span: SPAN,
                                name: Atom::from(var_name.as_str()),
                ...
                new_stmts.push(Statement::VariableDeclaration(Box::new_in(
                    VariableDeclaration {
                        span: SPAN,
                        kind: VariableDeclarationKind::Const,
...
impl<'a> DomExpressions<'a> {
    ...
    fn transform_jsx_element_modern(&mut self, jsx_elem: &JSXElement<'a>) -> Option<Expression<'a>> {
        ...
        let template = build_template_with_options(jsx_elem, &self.options);
        ...
        self.optimizer.record_template(&html, template.has_dynamic_content());
        self.templates.push(template.clone());

        // For now, create a simple IIFE with $clone
        let mut statements = OxcVec::new_in(self.allocator);

        // const _root$ = $clone(_tmpl$);
        let clone_call = self.create_clone_call(&template_var);
        let root_var = "_root$";
        ...
        // TODO: Add $bind calls for dynamic content
        // For now, we just return the root without any bindings
